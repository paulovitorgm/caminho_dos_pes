...
class Cadastro_de_paciente(forms.ModelForm):
    ...
    telefone = forms.CharField(label='Telefone', max_length=11, widget=forms.TextInput(attrs={'placeholder':'(XX) XXXXX-XXXX','autocomplete': 'off', 'class':'form-control' }))
    primeiro_atendimento = forms.DateField(label='Data do primeiro atendimento', required=False, widget=forms.DateInput(attrs={'type':'date','autocomplete': 'off', 'class':'form-control' }),)
    email = forms.EmailField(label='Email', max_length=150, required=False ,widget=forms.EmailInput(attrs={'placeholder':'email@email.com','autocomplete': 'off', 'class':'form-control' }))
    sexo_op = [('M' , 'Masculino'),('F' , 'Feminino')]
    sexo = forms.ChoiceField(label='Sexo', choices=sexo_op, widget=forms.Select(attrs={'class': 'form-select'}))
...
class Anamnese(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ['acompanhamento_medico', 'especialidade', 'uso_medicacao', 
                  'medicamento_em_uso', 'diabetico','hepatite', 'hiv',
                  'problemas_circulatorios','alergico', 'alergia_a', 
                  'teve_cancer', 'tipo_cancer', 'gravidez', 'lactante',
                  'hipertensao', 'hipotensao', 'observacoes']
